 ...
 */
    ...
    function ldapIdentify( $user=null, $conditions=null ) {

        //bind credentials against ldap
        $ldapUser = $this->_ldapAuth( $user );
        $model =& $this->getModel();
    ...
    function dbIdentify( $user ){
            ...
            $array = $this->hashPasswords( $array );
            $user[$this->userModel.".".$this->fields['password']] = $array['User']['password'];


            return $this->identify( $user,  $conditions );
    }
    ...
    function setLdapData( $data ) {
        ...
        $this->Session->write('LdapData', $data);
    }
