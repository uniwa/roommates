...
class OutputDriverFPDF extends OutputDriverGenericPDF {
  ...
  function image_rx($image, $x, $y, $width, $right, $ox, $oy, $scale) {
    ...
    while ($cx < $right) {
      ...
      $this->pdf->PdfImage($tmpname, $tx, $ty, $image->sx() * $scale, $image->sy() * $scale, "png");
      $cx += $width;
    };
  ...
  function image_scaled($image, $x, $y, $scale_x, $scale_y) {
    ...
    $this->_coords2pdf($x, $y);
    $this->pdf->PdfImage($tmpname, $x, $y - $image->sy() * $scale_y, $image->sx() * $scale_x, $image->sy() * $scale_y, "png");
    unlink($tmpname);
  }
